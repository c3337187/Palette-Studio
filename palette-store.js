(function(global){
  ...
  const ALIAS_DEFS = [
    {variable:'--surface-primary', label:'Поверхность основная', description:'Главный фон интерфейса.', info:'Используется как базовый холст интерфейса: фон страницы, крупных секций и общих контейнеров. Должна быть самой светлой поверхностью по сравнению с --surface-card и --surface-muted, чтобы сохранять читаемость и поддерживать контраст текста 4.5:1. Пример: основной фон дашборда, на котором лежат карточки — за счёт разницы яркости они остаются заметными и не сливаются с фоном.', defaultConfig:()=>({mode:'base', ref:'--color-base'})},
    {variable:'--surface-card', label:'Поверхность карточек', description:'Карточки и блоки.', info:'Применяется к карточкам контента, модальным окнам и панелям. Должна быть на шаг темнее или насыщеннее, чем --surface-primary, чтобы создавать визуальную иерархию и подчёркивать содержимое. Пример: карточка анонса или окно авторизации на светлом фоне — слегка более плотная подложка помогает отделить блок от основного холста.', defaultConfig:()=>({mode:'auto'}), auto:(base)=>mixColors(base['--color-base'], '#FFFFFF', 0.15)},
    {variable:'--surface-muted', label:'Поверхность приглушённая', description:'Вторичные подложки.', info:'Третий слой поверхности для вспомогательных элементов: состояния hover, фильтры, боковые панели. Должна быть заметно темнее или холоднее, чем --surface-card, но мягче, чем акцентные поверхности, чтобы не спорить с контентом. Пример: фон строки списка при наведении — более глубокий оттенок вторичного цвета подчёркивает взаимодействие, оставаясь в рамках системы.', defaultConfig:()=>({mode:'auto'}), auto:(base)=>mixColors(base['--color-secondary'], '#FFFFFF', 0.65)},
