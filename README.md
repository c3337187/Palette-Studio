...
* **Через CLI.** Скрипт `palette-generator.js` можно запустить из консоли:
  ...
  ```bash
  node palette-generator.js "#FFB870"
  ```

  Ответ — JSON с базовыми токенами (`--color-base`, `--color-secondary`, `--color-accent`, `--color-neutral`), набором alias и блоком `fonts` (главный, заголовочный и UI-шрифты). Этот JSON удобно прикладывать к задачам или использовать как подсказку при подготовке итоговой конфигурации.
...
#### Как именно строится профиль

Алгоритм считает, что введённый цвет — это доминирующий тон интерфейса (≈60%). Дальше он шаг за шагом достраивает остальные роли вокруг него:

1. **Нормализация входа.** HEX приводится к формату `#RRGGBB` и переводится в HSL, чтобы можно было управлять тоном, насыщенностью и светлотой.
2. **Основной блок (60%).** `--color-base` получается смешением выбранного оттенка с белым: фон остаётся окрашенным, но становится достаточно лёгким для больших поверхностей.
3. **Поддержка (30%).** `--color-secondary` — аналогичный тон, сдвинутый по кругу цвета примерно на ±30° и немного осветлённый. Он даёт мягкий контраст для карточек и второстепенных блоков.
4. **Акцент (10%).** `--color-accent` берётся из комплементарного тона (≈180°), усиливается по насыщенности и уравновешивается по светлоте, чтобы кнопки и ссылки сразу выделялись.
5. **Нейтраль.** `--color-neutral` автоматически выбирается тёмным или светлым: если фон светлый — берётся глубокий графит, если фон тёмный — светлый тон с запасом контраста.
6. **Alias-переменные.** Поверх базовых токенов строятся поверхности, кнопки, бейджи и границы. Все они получают значения из смесей базовых цветов (через `mixColors`), поэтому палитра остаётся цельной.
7. **Шрифтовой пресет.** По полученному тону выбирается пара гарнитур (заголовок/текст/UI) с поддержкой кириллицы. Этот набор можно заменить в любой момент.

В результате выбранный вами цвет реально становится «тем самым 60%», а генератор подбирает к нему вторичный и акцентный оттенки вместе с готовыми alias. Дальше профиль можно править вручную — Palette Studio сохраняет итоговые значения и не привязывает их к формуле.
    ...
    "profiles": [
      {
        ...
        "base": {"--color-base": "#RRGGBB", "--color-secondary": "#RRGGBB", "--color-accent": "#RRGGBB", "--color-neutral": "#RRGGBB"},
        "aliases": {
...
|------------|------------|
| `--color-base` | Основной фон и крупные поверхности (≈60% площади). |
| `--color-secondary` | Вторичные блоки, карточки и подложки (≈30%). |
| `--color-accent` | Акцентные элементы, CTA и активные ссылки (≈10%). |
| `--color-neutral` | Тёмный/светлый нейтральный для текста и иконок. |
...
#### Расширенные подсказки для alias

- **`--surface-primary`** — главный фон страницы, собирается из `--color-base`. Держите его светлее `--surface-card` минимум на 8–10 % по яркости и проверяйте контраст с `--text-strong` (≥ 4.5:1) и `--text-muted` (≥ 3:1).
- **`--surface-card`** — фон карточек и модулей на базе `--color-secondary`. Он должен быть плотнее `--surface-primary`, но чуть светлее `--surface-muted`, чтобы слои не слипались и контент оставался читаемым.
- **`--surface-muted`** — мягкая подложка для hover-состояний и вторичных панелей. Делайте её темнее, чем `--surface-card`, но спокойнее `--surface-accent` и `--action-primary`, сохраняя контраст ≥ 3:1 с `--text-muted`.
- **`--surface-hero`** — витринный блок, смешение `--color-secondary` и `--color-accent`. Поддерживайте его чуть светлее `--action-primary`, чтобы кнопка не терялась и сохраняла ведущую роль.
- **`--surface-accent`** — акцентные секции и промо-панели. Тон находится между `--surface-muted` и `--action-primary`; убедитесь, что белый или тёмный текст даёт контраст ≥ 4.5:1.
- **`--text-strong`** — основной текст, наследует `--color-neutral`. Это самый контрастный текстовый цвет, на котором проверяется соответствие WCAG (≥ 4.5:1 или ≥ 7:1 для заголовков).
- **`--text-muted`** — вторичные подписи, рассчитывается из `--color-neutral` и `--surface-primary`. Он заметно светлее `--text-strong`, но должен оставаться читаемым (≥ 3:1).
- **`--border-subtle`** — деликатные границы и разделители. Цвет чуть темнее `--surface-card`, но мягче акцентных линий, чтобы структура считывалась без жёстких контрастов.
- **`--action-primary`** — главный CTA, строится на базе `--color-accent` с добавлением `--color-neutral`. Требуемый контраст: ≥ 4.5:1 с текстом кнопки и ≥ 3:1 с поверхностями.
- **`--action-primary-hover`** — состояние наведения для CTA, осветлённый вариант `--action-primary`. Должен подчёркивать интерактивность и не конфликтовать с `--surface-accent`.
- **`--badge-accent`** — фон бейджей и тегов. Осветлённый `--color-secondary`/`--color-accent`, отличающийся от `--action-primary`, чтобы маркеры не напоминали кнопки.
...
* `profiles` — массив объектов. Каждый объект обязан содержать:
  ...
  * `base` — четыре ключевых токена (`--color-base`, `--color-secondary`, `--color-accent`, `--color-neutral`) в формате HEX `#RRGGBB`.
  ...
  "profiles": [
    {
      ...
      "aliases": {
        "--surface-primary": { "mode": "base", "ref": "--color-base" },
        "--surface-card": { "mode": "auto" },
        "--surface-muted": { "mode": "auto" },
        "--surface-hero": { "mode": "auto" },
        "--surface-accent": { "mode": "auto" },
        "--text-strong": { "mode": "base", "ref": "--color-neutral" },
        "--text-muted": { "mode": "auto" },
        "--border-subtle": { "mode": "auto" },
        "--action-primary": { "mode": "auto" },
        "--action-primary-hover": { "mode": "auto" },
        "--badge-accent": { "mode": "auto" }
      }
    }
  ],
  "fonts": {
...
Готовый файл с этим примером лежит в [`profiles/sample-profile.json`](profiles/sample-profile.json).
