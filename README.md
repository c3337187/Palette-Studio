...
Алгоритм не «угадывает» весь бренд под один клик и не смотрит на тренды в интернете. Он делает конкретные вычисления из выбранного основного цвета:

1. **Нормализация входа.** HEX приводится к формату `#RRGGBB`, переводится в HSL для дальнейших расчётов.
2. **Вывод базовых токенов.**
   * `--color-base` — осветлённый вариант выбранного цвета, пригодный для больших поверхностей (≈60%).
   * `--color-secondary` — оттенок того же тона, но чуть теплее/холоднее и светлее, чтобы поддержать «30%» интерфейса.
   * `--color-accent` — цвет, сдвинутый по тону примерно на 200°, усиленный по насыщенности и светлоте: он контрастирует с базой и подходит для CTA (≈10%).
   * `--color-neutral` — автоматически тёмный или светлый в зависимости от светлоты базового фона, чтобы текст оставался читабельным.
3. **Alias-переменные.** Поверх базовых токенов строятся остальные поверхности, кнопки, границы и т. д. Для них берутся смеси базовых цветов (через `mixColors`), чтобы палитра оставалась внутри одной гаммы без «внезапных» тонов.
4. **Шрифтовой пресет.** По цвету определяется подходящая пара гарнитур (например, для тёплых и светлых тонов — одна, для тёмных и холодных — другая), которую можно при желании заменить.

То есть генератор помогает стартовать с согласованного набора цветов, но не выбирает один цвет за «главный», а другие — «просто по трендам». Уже после генерации можно вручную скорректировать любой токен: Palette Studio хранит профиль, не привязывая его к автоматическому расчёту.
